...
describe "Neo4j::Cypher" do
  ...
  describe "DSL   { x = node; n = node(3); match n <=> x; ret x }" do
    it { Proc.new { x = node; n = node(3); match n <=> x; ret x }.should be_cypher("START v2=node(3) MATCH (v2)--(v1) RETURN v1") }
  end
  ...
  describe "DSL   { node(3) > :r > :x; :r }" do
    it { Proc.new { node(3) > :r > :x; :r }.should be_cypher("START v1=node(3) MATCH (v1)-[r]->(x) RETURN r") }
  end
  ...
  describe "DSL   { r = rel('r:friends').as(:r); node(3) > r > :x; r }" do
    it { Proc.new { r = rel('r:friends').as(:r); node(3) > r > :x; r }.should be_cypher("START v1=node(3) MATCH (v1)-[r:friends]->(x) RETURN r") }
  end
  ...
  describe "DSL   { node(3) > :r > node; node }" do
    it { Proc.new { node(3) > :r > node; :r }.should be_cypher("START v1=node(3) MATCH (v1)-[r]->(v3) RETURN r") }
  end
  ...
  describe %{n=node(3, 1); where((n[:age] < 30) & ((n[:name] == 'foo') | (n[:size] > n[:age]))); ret n} do
    it { Proc.new { n=node(3, 1); where((n[:age] < 30) & ((n[:name] == 'foo') | (n[:size] > n[:age]))); ret n }.should be_cypher(%q[START v1=node(3,1) WHERE (v1.age < 30) and ((v1.name = "foo") or (v1.size > v1.age)) RETURN v1]) }
  end
  ...
  describe %{ a, x=node(1), node(2); p = shortest_path { a > '?*' > x }; p } do
    it { Proc.new { a, x=node(1), node(2); p = shortest_path { a > '?*' > x }; p }.should be_cypher(%{START v1=node(1),n1=node(2) MATCH m3 = shortestPath((v1)-[?*]->(n1)) RETURN m3}) }
  end

  describe %{shortest_path{node(1) > '?*' > node(2)}} do
    it { Proc.new { shortest_path { node(1) > '?*' > node(2) } }.should be_cypher(%{START v1=node(1),n2=node(2) MATCH m3 = shortestPath((v1)-[?*]->(n2)) RETURN m2}) }
  end

  describe %{shortest_path { node(1) > '?*' > :x > ':friend' > node(2)}} do
    it { Proc.new { shortest_path { node(1) > '?*' > :x > ':friend' > node(2) } }.should be_cypher(%{START v1=node(1),n2=node(2) MATCH m3 = shortestPath((v1)-[?*]->(x)-[:friend]->(n2)) RETURN m2}) }
  end
  ...
  describe "a=node(3); a < ':knows' < node(:c) < :friends < :d; :friends" do
    it { Proc.new { a=node(3); a < ':knows' < node(:c) < :friends < :d; :friends }.should be_cypher(%{START v1=node(3) MATCH (v1)<-[:knows]-(c)<-[friends]-(d) RETURN friends}) }
  end
  ...
  describe %{node(3) >> :b} do
    it { Proc.new { node(3) >> :b }.should be_cypher(%{START v1=node(3) MATCH m2 = (v1)-->(b) RETURN m2}) }
  end
  ...
  describe "node(1).both(node(:other_person))" do
    it do
      ...
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[?]-(other_person) RETURN other_person])
    end
  end
  ...
  describe "node(1).both(node(:other_person)).both(node(:foo))" do
    it do
      ...
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[?]-(other_person),(other_person)-[?]-(foo) RETURN foo])
    end
  end
  ...
  describe "node(1).both(:friends)" do
    it do
      Proc.new do
        node(1).both(:friends)
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[:`friends`]-(v1) RETURN v1])
    end
  end
  ...
  describe "node(1).both(:friends, :work)" do
    it do
      ...
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[:`friends`|`work`]-(v1) RETURN v1])
    end
  end
  ...
  describe "node(1).both(:friends, :work, node(42)).both" do
    it do
      ...
      end.should be_cypher(%Q[START v1=node(1),n1=node(42) MATCH (v1)-[:`friends`|`work`]-(n1),(n1)-[?]-(v1) RETURN v1])
    end
  end
  ...
  describe "node(1).both?(:friends)" do
    it do
      Proc.new{node(1).both?(:friends)}.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[?:`friends`]-(v1) RETURN v1])
    end
  end

  describe "node(1).both('r:friends')" do
    it do
      Proc.new{node(1).both('r:friends')}.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[r:friends]-(v1) RETURN v1])
    end
  end

  describe "node(1).both?(:friends)" do
    it {Proc.new{node(1).both?(:friends)}.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[?:`friends`]-(v1) RETURN v1])}
  end

  describe "node(1).outgoing?(:friends)" do
    it { Proc.new { node(1).outgoing?(:friends)}.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[?:`friends`]->(v1) RETURN v1]) }
  end

  describe "node(1).incoming?(:friends)" do
    it { Proc.new { node(1).incoming?(:friends)}.should be_cypher(%Q[START v1=node(1) MATCH (v1)<-[?:`friends`]-(v1) RETURN v1]) }
  end

  describe %{p = node(3) >> :b; [:b, p.length]} do
    it { Proc.new { p = node(3) >> :b; [:b, p.length] }.should be_cypher(%{START v1=node(3) MATCH m2 = (v1)-->(b) RETURN b,length(m2)}) }
  end
  ...
  describe %{DSL    (n = node(2))>>:x; count} do
    it { Proc.new { (n = node(2))>>:x; count }.should be_cypher(%{START v1=node(2) MATCH (v1)-->(x) RETURN count(*)}) }
  end
  ...
  describe "        a = node(3); b=node(1); match p = a > '*1..3' > b; where p.nodes.all? { |x| x[:age] > 30 }; ret p" do
    it { Proc.new { a = node(3); b=node(1); match p = a > '*1..3' > b; where p.nodes.all? { |x| x[:age] > 30 }; ret p }.should be_cypher(%{START v1=node(3),n1=node(1) MATCH m3 = (v1)-[*1..3]->(n1) WHERE all(x in nodes(m3) WHERE x.age > 30) RETURN m3}) }
  end
  ...
  describe "        p=node(3)>'*1..3'>:b; p.nodes.none? { |x| x[:age] == 25 };p" do
    it { Proc.new { p=node(3)>'*1..3'>:b; p.nodes.none? { |x| x[:age] == 25 }; p }.should be_cypher(%{START v1=node(3) MATCH m2 = (v1)-[*1..3]->(b) WHERE none(x in nodes(m2) WHERE x.age = 25) RETURN m2}) }
  end

  describe %{       p = node(3)>>:b; p.nodes.single? { |x| x[:eyes] == 'blue' }; p } do
    it { Proc.new { p = node(3)>>:b; p.nodes.single? { |x| x[:eyes] == 'blue' }; p }.should be_cypher(%{START v1=node(3) MATCH m2 = (v1)-->(b) WHERE single(x in nodes(m2) WHERE x.eyes = "blue") RETURN m2}) }
  end

  describe %{       p = node(3)>>:b; p.rels.single? { |x| x[:eyes] == 'blue' }; p } do
    it { Proc.new { p = node(3)>>:b; p.rels.single? { |x| x[:eyes] == 'blue' }; p }.should be_cypher(%{START v1=node(3) MATCH m2 = (v1)-->(b) WHERE single(x in relationships(m2) WHERE x.eyes = "blue") RETURN m2}) }
  end

  describe %{       a=node(3); b=node(4); c=node(1); p=a>>b>>c; p.nodes.extract { |x| x[:age] }} do
    it { Proc.new { a=node(3); b=node(4); c=node(1); p=a>>b>>c; p.nodes.extract { |x| x[:age] } }.should be_cypher(%{START v1=node(3),n1=node(4),n2=node(1) MATCH m4 = (v1)-->(n1)-->(n2) RETURN extract(x in nodes(m4) : x.age)}) }
  end
  ...
  describe %{       a=node(3); b = node(2); ret a[:age], b[:age], (a[:age] - b[:age]).abs } do
    it { Proc.new { a=node(3); b = node(2); ret a[:age], b[:age], (a[:age] - b[:age]).abs }.should be_cypher(%{START v1=node(3),n1=node(2) RETURN v1.age,n1.age,abs(v1.age - n1.age)}) }
  end

  describe %{       a=node(3); b = node(2); ret (a[:age] - b[:age]).abs.as("newname") } do
    it { Proc.new { a=node(3); b = node(2); ret (a[:age] - b[:age]).abs.as("newname") }.should be_cypher(%{START v1=node(3),n1=node(2) RETURN abs(v1.age - n1.age) AS newname}) }
  end
  ...
  describe %{       n=node(3,1,2); p=node(5,6); ret(n).asc(p[:age]).desc(n[:name]) } do
    it { Proc.new { n=node(3, 1, 2); p=node(5, 6); ret(n).asc(p[:age]).desc(n[:name]) }.should be_cypher(%{START v1=node(3,1,2),n1=node(5,6) RETURN v1 ORDER BY n1.age, v1.name DESC}) }
  end
  ...
  describe %{       a=node(3); c = node(2); p = a >> :b >> c; nodes(p) } do
    it { Proc.new { a=node(3); c = node(2); p = a >> :b >> c; nodes(p) }.should be_cypher(%{START v1=node(3),n1=node(2) MATCH m3 = (v1)-->(b)-->(n1) RETURN nodes(m3)}) }
  end


  describe %{       a=node(3); c = node(2); p = a >> :b >> c; rels(p) } do
    it { Proc.new { a=node(3); c = node(2); p = a >> :b >> c; rels(p) }.should be_cypher(%{START v1=node(3),n1=node(2) MATCH m3 = (v1)-->(b)-->(n1) RETURN relationships(m3)}) }
  end
  ...
  describe "5.2. Basic Friend finding based on social neighborhood " do
    ...
    it "also works with outgoing method instead of < operator" do
      Proc.new do
        ...
        r.exist?
        ret(friends_of_friends[:name], count).desc(count).asc(friends_of_friends[:name])
      end.should be_cypher(%{START v1=node(3) MATCH (v1)-[:`knows`]->(v1),(v1)-[:`knows`]->(v2),(v1)-[r?:knows]->(v2) WHERE (r is null) RETURN v2.name,count(*) ORDER BY count(*) DESC, v2.name})
    end
  ...
  describe "using model classes and declared relationship" do
    it "escape relationships name and allows is_a? instead of [:_classname] = klass" do
      ...
      Proc.new do
        ...
        rc.is_a?(RC)
        rc
      end.should be_cypher(%{START v1=node(2),n1=node(3) MATCH (v1)-[:`User#rc`]->(rc)<-[:`Place#rs`]-(n1),(rc)<-[:`active`]-(v4) WHERE rc._classname = "RC" RETURN rc})
    end
  end
  ...
  describe "a=node(5);b=node(7);x=node; a > ':friends' > x; (x > ':friends' > node > ':work' > b).not; x" do
    it do
      ...
      end.should be_cypher("START v1=node(5),n1=node(7) MATCH (v1)-[:friends]->(v1) WHERE not((v1)-[:friends]->(v2)-[:work]->(n1)) RETURN v1")
    end
  end
  ...
  describe "(node(5) > :r > :middle) >> node(7)" do
    it do
      ...
      end.should be_cypher("START v1=node(5),n2=node(7) MATCH m3 = (v1)-[r]->(middle)-->(n2) RETURN m3")
    end
  end
  ...
  describe "node(1) > (r=rel(:friends)) > :other; r[:since] == 1994; :other" do
    it do
      ...
      end.should be_cypher("START v1=node(1) MATCH (v1)-[v2:`friends`]->(other) WHERE v2.since = 1994 RETURN other")
    end
  end
  ...
  describe "node(1) > (rel(:knows)[:since] == 1994) > :other; :other" do
    it do
      ...
      end.should be_cypher("START v1=node(1) MATCH (v1)-[v2:`knows`]->(other) WHERE v2.since = 1994 RETURN other")
    end
  end
  ...
  describe "node(1) > (rel(:knows)[:since] > 1994) > (node(:other)[:name] == 'foo'); :other"do
    it do
      ...
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)-[v2:`knows`]->(other) WHERE v2.since > 1994 and other.name = "foo" RETURN other])
    end
  end
  ...
  describe "node.new" do
    it do
      Proc.new do
        node.new
      end.should be_cypher(%Q[ CREATE (v1)])
    end
  end
  ...
  describe "node.new(:name => 'Andres', :age => 42)" do
    it do
      ...
      end.should be_cypher(%Q[ CREATE (v1 {name : 'Andres', age : 42})])
    end
  end
  ...
  describe "a = node(1).as(:a); b = node(2).as(:b); create_path{a > rel(:friends, :_name => \"a.name + '<->' + b.name\").as(:r) > b} :r" do
    it do
        ...
        a = node(1).as(:a)
        b = node(2).as(:b)
        create_path{puts "Hello #{expressions.object_id}"; a > rel(:friends, :_name => "a.name + '<->' + b.name").as(:r) > b}
        :r
  ...
  describe "create_path{node.new(:name => 'Andres') > rel(:WORKS_AT) > node < rel(:WORKS_AT) < node.new(:name => 'Micahel')}" do
    it do
      ...
      end.should be_cypher(%Q[ CREATE p0 = (v1 {name : 'Andres'})-[:`WORKS_AT`]->(v2)<-[:`WORKS_AT`]-(v4 {name : 'Micahel'}) RETURN p0])
    end
  end
  ...
  describe "node(1) <=> node(:other_person) >> node}; with(:other_person, count){|_, foaf| foaf > 1} :other_person " do
    it do
      Proc.new do
        ...
        :other_person
      end.should be_cypher(%Q[START v1=node(1) MATCH (v1)--(other_person)-->(v2) WITH other_person,count(*) as w2 WHERE w2 > 1 RETURN other_person])
    end
  end


  describe "n = node(1); n > rel(:knows) > :other; create_path(:other){|other| other > rel(:works) > node }" do
    it do
      Proc.new do
        n = node(1); n > rel(:knows) > :other; create_path(:other){|other| other > rel(:works) > node }
      end.should be_cypher(%Q["START n=node(1) MATCH n-[:KNOWS]-other WITH other CREATE n-[:`WORKS`]->other"])
    end
  end
